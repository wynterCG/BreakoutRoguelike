...
func _ready() -> void:
	...
	var block_size: Vector2 = Vector2(BLOCK_WIDTH, BLOCK_HEIGHT)

	if monster_data:
		hp = monster_data.hp
		block_color = monster_data.color
		block_size = monster_data.size
	else:
	...
	_background.size = block_size
	_background.position = Vector2(-block_size.x / 2.0, -block_size.y / 2.0)
	...
	_fill_bar.size = Vector2(block_size.x - BAR_PADDING * 2.0, block_size.y - BAR_PADDING * 2.0)
...
func _update_visual() -> void:
	...
	var total_hp: int = max_hp
	if monster_data:
		total_hp = monster_data.hp

	var block_size: Vector2 = Vector2(BLOCK_WIDTH, BLOCK_HEIGHT)
	if monster_data:
		block_size = monster_data.size

	var inner_width: float = block_size.x - BAR_PADDING * 2.0
	var hp_ratio: float = float(current_hp) / float(total_hp)
